...
void draw_pointcloud(float width, float height, glfw_state& app_state, rs2::points& points, float point_size)
{
    if (!points)
        return;
...
void draw_pointcloud(float width, float height, glfw_state& app_state, rs2::points& points, std::vector<rs2::vertex>& vertices, float point_size)
{
    if (!points)
        return;
    ...
    {
        /* this segment actually prints the pointcloud */
        auto _vertices = points.get_vertices();              // get vertices
        auto tex_coords = points.get_texture_coordinates(); // and texture coordinates
        for (int i = 0; i < points.size(); i++)
        {
            if (_vertices[i].z)
