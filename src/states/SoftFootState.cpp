...
void SoftFootState::start()
{
  ...
  ctl.datastore().make_call("SoftFootState::GetState",
    [this]() -> std::string { return current_state_; });
...
void SoftFootState::runState()
{
  ...
  if(!foot_data_[current_moving_foot].computation_done && enough_ground_in_front && enough_ground_in_back
     && ctl.swingTraj->t_ > ctl.swingTraj->startTime_ + (ctl.swingTraj->endTime_ - ctl.swingTraj->startTime_) * 0.5)
  {
    ...
    // Extract ground segment from ground data
    {
      ...
      auto stop = std::chrono::high_resolution_clock::now();
      auto duration = std::chrono::duration<double, std::milli>(stop - start);
      mc_rtc::log::info("[SoftFootState] extractGroundSegment(ctl, current_moving_foot, X_0_landing.translation()) took {:.4f}ms", duration.count());
    }
    ...
    if(!ground_segment_[current_moving_foot].raw.empty())
    {
      if(ground_segment_[current_moving_foot].raw.back().x() - ground_segment_[current_moving_foot].raw.front().x() > 0.25 * foot_length_)
      {
        // Compute the altitude profile
        {
          ...
          auto stop = std::chrono::high_resolution_clock::now();
          auto duration = std::chrono::duration<double, std::milli>(stop - start);
          mc_rtc::log::info("[SoftFootState] extractAltitudeProfileFromGroundSegment(current_moving_foot) took {:.4f}ms", duration.count());
        }
        ...
        // Compute convex hull of the segment -> right now it does not work
        {
          ...
          auto stop = std::chrono::high_resolution_clock::now();
          auto duration = std::chrono::duration<double, std::milli>(stop - start);
          mc_rtc::log::info("[SoftFootState] computeSegmentConvexHull(ctl, current_moving_foot) took {:.4f}ms", duration.count());
        }
        ...
        if(with_foot_adjustment_)
        {
          // Compute min/max phalanxes angle depending of stiffness
          {
            ...
            auto stop = std::chrono::high_resolution_clock::now();
            auto duration = std::chrono::duration<double, std::milli>(stop - start);
            mc_rtc::log::info("[SoftFootState] computeMinMaxAngle(current_moving_foot) took {:.4f}ms", duration.count());
          }
          // Compute the landing position
          {
            ...
            auto stop = std::chrono::high_resolution_clock::now();
            auto duration = std::chrono::duration<double, std::milli>(stop - start);
            mc_rtc::log::info("[SoftFootState] computeFootLandingPosition(current_moving_foot, X_0_landing.translation()) took {:.4f}ms", duration.count());
          }
          // Update landing pose
        ...
        if(with_ankle_rotation_)
        {
          // Now with the convex hull we can compute the angle
          {
            ...
            auto stop = std::chrono::high_resolution_clock::now();
            auto duration = std::chrono::duration<double, std::milli>(stop - start);
            mc_rtc::log::info("[SoftFootState] computeFootLandingAngle(current_moving_foot, X_0_landing.translation()) took {:.4f}ms", duration.count());
          }
        }
...
void SoftFootState::extractGroundSegment(mc_control::fsm::Controller & ctl, const Foot & current_moving_foot, const Eigen::Vector3d & landing)
{
  ...
  mc_rtc::log::info("[SoftFootState] ground_segment_[{}].raw.size() {}", current_moving_foot == Foot::Left ? "Left" : "Right", ground_segment_[current_moving_foot].raw.size());
  mc_rtc::log::info("[SoftFootState] ground_segment_[{}].filtered.size() {}", current_moving_foot == Foot::Left ? "Left" : "Right", ground_segment_[current_moving_foot].filtered.size());
...
void SoftFootState::computeFootLandingPosition(const Foot & current_moving_foot, const Eigen::Vector3d & default_landing_pos)
{
  ...
  auto computeSide = [&](int start, int end, double direction, const Eigen::Vector3d & phalanx_pos,
    double theta, std::vector<double>& ret_phalanxes_theta, std::vector<Eigen::Vector2d>& ret_contact_points,
...
void SoftFootState::updateFootSwingPose(mc_control::fsm::Controller & ctl, const Foot & current_moving_foot, const sva::PTransformd &)
{
  ...
  double desired_offset_position_z = foot_data_[current_moving_foot].position_offset_z * 1.15;
  if(desired_angle > 0.05)
  {
